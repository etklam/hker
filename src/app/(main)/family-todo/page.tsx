...
import type { FamilyTodoSpace } from '@/lib/types'
...
export default function FamilyTodoPage() {
  ...
  const load = useCallback(async () => {
    try {
      const data = await api<FamilyTodoSpace[]>('/api/family-todo/spaces')
      setSpaces(data)
    } catch {
      pushToast(t('familyTodo.errors.loadSpaces'), 'error')
    } finally {
      setLoading(false)
    }
  }, [t])
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    setCreating(true)
    try {
      const space = await api<FamilyTodoSpace>('/api/family-todo/spaces', {
        method: 'POST',
        body: { name: trimmed },
      })
      ...
      pushToast(t('familyTodo.errors.saveSpace'), 'error')
    } finally {
      setCreating(false)
    }
  }
  ...
  async function handleDelete(sid: number) {
    if (!confirm(t('familyTodo.deleteSpaceConfirm'))) return
    try {
      await api(`/api/family-todo/spaces/${sid}`, { method: 'DELETE' })
      setSpaces((prev) => prev.filter((s) => s.id !== sid))
    } catch {
      pushToast(t('familyTodo.errors.deleteSpace'), 'error')
    }
  }
  ...
  async function handleRename(sid: number) {
    ...
    try {
      const updated = await api<FamilyTodoSpace>(`/api/family-todo/spaces/${sid}`, {
        method: 'PATCH',
        body: { name: trimmed },
      })
    ...
    <div className="mx-auto max-w-4xl px-4 py-8">
          ...
          <div className="mochi-card mx-4 w-full max-w-sm p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleCreate} className="flex flex-col gap-4">
              <label className="flex flex-col gap-1">
                <span className="text-sm font-medium text-text">{t('familyTodo.spaceName')}</span>
                <input
                  type="text"
                  value={newName}
      ...
      {spaces.length === 0 ? (
        <div className="py-16 text-center">
          <CheckSquare size={48} className="mx-auto mb-4 text-muted/40" />
          <p className="text-muted">{t('familyTodo.empty')}</p>
        </div>
      ) : (
          ...
          {spaces.map((space) => (
            ...
            >
              ...
              <div className="flex items-center gap-2 pt-1">
                <Link
                  href={`/family-todo/spaces/${space.id}`}
