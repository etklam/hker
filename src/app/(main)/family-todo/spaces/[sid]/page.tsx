...
export default function SpaceBoardPage() {
  ...
  const [space, setSpace] = useState<FamilyTodoSpace | null>(null)
  ...
  async function handleMoveTodo(todoId: number, targetListId: number) {
    const snapshot = structuredClone(lists)
    try {
      await api(`/api/family-todo/todos/${todoId}/move`, {
        method: 'PATCH',
        body: { targetListId },
      })
    } catch {
  ...
  async function handleAddTodo(
    ...
    try {
      const todo = await api<TodoResponse>(`/api/family-todo/lists/${listId}/todos`, {
        method: 'POST',
        body: data,
      })
      setLists((prev) =>
  ...
  async function handleAddList(title: string) {
    try {
      const list = await api<TodoListWithItems>(`/api/family-todo/spaces/${sid}/lists`, {
        method: 'POST',
        body: { title },
      })
  ...
  async function handleToggleTodo(todoId: number, completed: boolean) {
    ...
    try {
      await api(`/api/family-todo/todos/${todoId}/complete`, {
        method: 'PATCH',
        body: { completed },
      })
    } catch {
  ...
  async function handleDeleteList(listId: number) {
    ...
    try {
      await api(`/api/family-todo/spaces/${sid}/lists/${listId}`, { method: 'DELETE' })
    } catch {
      setLists(snapshot)
  ...
  async function handleUpdateListTitle(listId: number, title: string) {
    ...
    try {
      await api(`/api/family-todo/spaces/${sid}/lists/${listId}`, {
        method: 'PATCH',
        body: { title },
      })
    } catch {
  ...
  async function handleSaveTodo(data: {
    ...
    if (editingTodo) {
      // Update existing
      try {
        const updated = await api<TodoResponse>(`/api/family-todo/todos/${editingTodo.id}`, {
          method: 'PATCH',
          body: data,
        })
        setLists((prev) =>
  ...
  async function handleDeleteTodo() {
    ...
    setEditingTodo(null)
    try {
      await api(`/api/family-todo/todos/${editingTodo.id}`, { method: 'DELETE' })
    } catch {
      setLists(snapshot)
  ...
  async function loadMembers() {
    ...
    try {
      const [m, i] = await Promise.all([
        api<Member[]>(`/api/family-todo/spaces/${sid}/members`),
        api<InviteLink[]>(`/api/family-todo/spaces/${sid}/invites`),
      ])
      setMembers(m)
      setInvites(i)
    } catch {
  ...
  async function handleCreateInvite() {
    ...
    try {
      const invite = await api<InviteLink>(`/api/family-todo/spaces/${sid}/invites`, {
        method: 'POST',
        body: {},
      })
  ...
  async function handleRemoveMember(memberId: number) {
    ...
    try {
      await api(`/api/family-todo/spaces/${sid}/members/${memberId}`, { method: 'DELETE' })
  ...
  async function handleDeleteInvite(inviteId: number) {
    ...
    try {
      await api(`/api/family-todo/spaces/${sid}/invites/${inviteId}`, { method: 'DELETE' })
