...
import type { SpaceInviteInfo } from '@/lib/types'

export default function SpaceInviteJoinPage() {
  ...
  const load = useCallback(async () => {
    try {
      const data = await api<SpaceInviteInfo>(`/api/spaces/invites/${token}`)
      setInvite(data)
    } catch (e: unknown) {
  ...
  async function handleJoin() {
    setJoining(true)
    try {
      const result = await api<{ space: { id: number; name: string } }>(`/api/spaces/invites/${token}/join`, { method: 'POST' })
      pushToast(t('spaceInviteJoin.subtitle', { title: result.space.name }), 'success')
      router.push(`/spaces/spaces/${result.space.id}`)
    } catch {
      pushToast(t('spaceInviteJoin.errors.join'), 'error')
    } finally {
      setJoining(false)
    }
  }
  ...
  function handleLogin() {
    router.push(`/login?returnTo=/spaces/invite/${token}`)
  }
      ...
      <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] shadow-lg max-w-md w-full overflow-hidden">
        ...
        <div className="bg-[var(--accent-soft)] px-8 py-6 text-center">
          ...
          <h1 className="font-[family-name:var(--font-heading)] text-2xl font-extrabold text-text">
            {t('spaceInviteJoin.title')}
          </h1>
          <p className="mt-1 text-sm text-[var(--muted)]">
            {t('spaceInviteJoin.subtitle', { title: invite.space.name })}
          </p>
        </div>
          ...
          {invite.isValid ? (
            user ? (
              ...
              >
                ...
                {joining ? t('common.loading') : t('spaceInviteJoin.joinNow')}
              </button>
            ) : (
              <button
                type="button"
