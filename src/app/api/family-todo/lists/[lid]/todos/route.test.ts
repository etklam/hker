...
describe('POST /api/family-todo/lists/[lid]/todos', () => {
  let POST: any
  let todoService: any
  ...
  it('returns 400 for missing title', async () => {
    ...
    const res = await POST(makeReq('/api/family-todo/lists/1/todos', {}), { user })
    expect(res.status).toBe(400)
  })

  it('returns 400 for invalid priority', async () => {
    todoService.getSpaceIdForList.mockResolvedValue(1)
    const res = await POST(makeReq('/api/family-todo/lists/1/todos', { title: 'Task', priority: 'critical' }), { user })
    expect(res.status).toBe(400)
  })

  it('returns 404 for missing list', async () => {
    todoService.getSpaceIdForList.mockResolvedValue(null)
    const res = await POST(makeReq('/api/family-todo/lists/999/todos', { title: 'Task' }), { user })
  ...
  it('creates todo and returns 201', async () => {
    ...
    const res = await POST(makeReq('/api/family-todo/lists/1/todos', { title: 'Task' }), { user })
