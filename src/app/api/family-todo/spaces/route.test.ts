...
describe('/api/family-todo/spaces', () => {
  ...
  describe('GET', () => {
    it('returns spaces for user', async () => {
      spaceService.listForUser.mockResolvedValue([{ id: 1, name: 'Family' }])
      const res = await GET(new Request('http://localhost:3000/api/family-todo/spaces') as any, { user })
  ...
  describe('POST', () => {
    function makeReq(body: any) {
      return new Request('http://localhost:3000/api/family-todo/spaces', {
        method: 'POST',
    ...
    it('creates space and returns 201', async () => {
      spaceService.create.mockResolvedValue({
        id: 1, name: 'Family', ownerId: 1,
        createdAt: new Date(), updatedAt: new Date(),
      })
      const res = await POST(makeReq({ name: 'Family' }), { user })
