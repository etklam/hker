...
describe('/api/family-todo/spaces/[sid]/invites', () => {
  ...
  describe('GET', () => {
    it('returns invites with urls', async () => {
      ...
      const res = await GET(makeReq('/api/family-todo/spaces/1/invites'), { user })
      expect(res.status).toBe(200)
      const body = await res.json()
      expect(body[0].url).toContain('/family-todo/invite/abc')
    })
  })

  describe('POST', () => {
    it('returns 400 for invalid maxUses', async () => {
      const res = await POST(makeReq('/api/family-todo/spaces/1/invites', { maxUses: -1 }), { user })
      expect(res.status).toBe(400)
    })

    it('returns 400 for invalid expiresInHours', async () => {
      const res = await POST(makeReq('/api/family-todo/spaces/1/invites', { expiresInHours: 0 }), { user })
      expect(res.status).toBe(400)
    })
    ...
    it('creates invite and returns 201', async () => {
      ...
      const res = await POST(makeReq('/api/family-todo/spaces/1/invites', { maxUses: 5 }), { user })
