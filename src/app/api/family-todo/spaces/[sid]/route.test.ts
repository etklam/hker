...
describe('/api/family-todo/spaces/[sid]', () => {
  ...
  describe('PATCH', () => {
    it('updates space', async () => {
      ...
      const res = await PATCH(makeReq('/api/family-todo/spaces/1', { name: 'Updated' }), { user })
    ...
    it('returns 404 when space not found', async () => {
      ...
      const res = await PATCH(makeReq('/api/family-todo/spaces/999', { name: 'X' }), { user })
  ...
  describe('DELETE', () => {
    it('deletes space and returns 204', async () => {
      const res = await DELETE(makeReq('/api/family-todo/spaces/1'), { user })
