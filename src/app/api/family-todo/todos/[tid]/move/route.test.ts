...
describe('PATCH /api/family-todo/todos/[tid]/move', () => {
  let PATCH: any
  let todoService: any
  ...
  it('moves todo to another list', async () => {
    ...
    const res = await PATCH(makeReq('/api/family-todo/todos/1/move', { targetListId: 2 }), { user })
  ...
  it('returns 400 when targetListId is missing', async () => {
    todoService.getSpaceIdForTodo.mockResolvedValue(1)
    const res = await PATCH(makeReq('/api/family-todo/todos/1/move', {}), { user })
    expect(res.status).toBe(400)
  })

  it('returns 404 for missing todo', async () => {
    todoService.getSpaceIdForTodo.mockResolvedValue(null)
    const res = await PATCH(makeReq('/api/family-todo/todos/999/move', { targetListId: 2 }), { user })
