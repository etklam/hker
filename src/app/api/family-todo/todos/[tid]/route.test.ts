...
vi.mock('@/server/services/family-todo-board-service', () => ({
  buildTodoResponse: vi.fn(),
}))

describe('/api/family-todo/todos/[tid]', () => {
  ...
  describe('PATCH', () => {
    it('updates todo', async () => {
      ...
      const res = await PATCH(makeReq('/api/family-todo/todos/1', { title: 'Updated' }), { user })
    ...
    it('returns 400 for invalid priority', async () => {
      todoService.getSpaceIdForTodo.mockResolvedValue(1)
      const res = await PATCH(makeReq('/api/family-todo/todos/1', { priority: 'critical' }), { user })
      expect(res.status).toBe(400)
    })

    it('returns 404 for missing todo', async () => {
      todoService.getSpaceIdForTodo.mockResolvedValue(null)
      const res = await PATCH(makeReq('/api/family-todo/todos/999', { title: 'X' }), { user })
  ...
  describe('DELETE', () => {
    it('deletes todo and returns 204', async () => {
      todoService.getSpaceIdForTodo.mockResolvedValue(1)
      const res = await DELETE(makeReq('/api/family-todo/todos/1'), { user })
    ...
    it('returns 404 for missing todo', async () => {
      todoService.getSpaceIdForTodo.mockResolvedValue(null)
      const res = await DELETE(makeReq('/api/family-todo/todos/999'), { user })
      expect(res.status).toBe(404)
    })
  })
})
