...
describe('PATCH /api/family-todo/todos/[tid]/complete', () => {
  let PATCH: any
  let todoService: any
  ...
  it('toggles complete', async () => {
    ...
    const res = await PATCH(makeReq('/api/family-todo/todos/1/complete', { completed: true }), { user })
  ...
  it('returns 400 when completed is not boolean', async () => {
    todoService.getSpaceIdForTodo.mockResolvedValue(1)
    const res = await PATCH(makeReq('/api/family-todo/todos/1/complete', { completed: 'yes' }), { user })
    expect(res.status).toBe(400)
  })

  it('returns 404 for missing todo', async () => {
    todoService.getSpaceIdForTodo.mockResolvedValue(null)
    const res = await PATCH(makeReq('/api/family-todo/todos/999/complete', { completed: true }), { user })
