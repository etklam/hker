...
describe('GET /api/spaces/invites/[token]', () => {
  let GET: any
  ...
  it('returns invite details', async () => {
    ...
    const res = await GET(makeReq('/api/spaces/invites/abc123'), { user: null })
  ...
  it('returns 404 for missing invite', async () => {
    ...
    const res = await GET(makeReq('/api/spaces/invites/nonexistent'), { user: null })
    expect(res.status).toBe(404)
  })

  it('returns 429 when rate limited', async () => {
    mockRateLimit.mockResolvedValue({ allowed: false, retryAfterMs: 30000 })
    const res = await GET(makeReq('/api/spaces/invites/abc'), { user: null })
  ...
  it('marks expired invite as invalid', async () => {
    ...
    const res = await GET(makeReq('/api/spaces/invites/expired'), { user: null })
    const body = await res.json()
