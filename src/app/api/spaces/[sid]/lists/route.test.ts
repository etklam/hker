...
describe('POST /api/spaces/[sid]/lists', () => {
  let POST: any
  let todoService: any
  const user = { id: 1 }
  ...
  it('creates list and returns 201', async () => {
    ...
    const res = await POST(makeReq('/api/spaces/1/lists', { title: 'Groceries' }), { user })
  ...
  it('returns 400 for missing title', async () => {
    const res = await POST(makeReq('/api/spaces/1/lists', {}), { user })
